...
3. ~~**Port the remaining five lidr signals to Python**: `rsi`, `macd`, `bollinger`, `breakout`, `volume`.~~ ✅ Done 2026-05-27 in PRs #5 (RSI), #7 (MACD), #8 (Bollinger), #9 (breakout), #10 (volume). Each signal verified against lidr's TS on real SPY data — four exact bit-match, one machine-precision (1.5e-11 max diff for Bollinger due to streaming-vs-direct std accumulation). Layer-1 reference + layer-2 hand-derived spot checks added for each. The Python pipeline now exposes six signals (the five above plus the pre-existing `sma_crossover`). *Next: re-run the logistic baseline on all six as a cheap checkpoint — if six features still can't beat buy-and-hold, the bottleneck is the model, not the features, and the answer is #4 (LightGBM).*
...
All five signal ports (Next Up #3) shipped 2026-05-27 — the pipeline now has six features available. Next: re-run the logistic-regression baseline on all six features (sma_crossover + rsi + macd + bollinger + breakout + volume) as a cheap checkpoint. If six features still can't beat buy-and-hold, skip to #4 (LightGBM); if they do beat it, the edge gate opens and items #7–#11 unlock.
