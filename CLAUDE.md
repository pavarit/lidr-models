...
- **Step 3 — PR-C** (next, planned 2026-05-30): Real `news_v0.yaml` backtest on a 7-ticker high-news universe (NVDA, TSLA, GME, AMC, PLTR, MU, USO); EODHD+GDELT+EDGAR, FinBERT-only, h5 binary, logistic, one config per ticker via `make backtest-news-v0`; `results_log` row + refreshed manifest + comparison (vs h5 logistic ta_ensemble re-run on the same tickers + buy-and-hold) per the evidence convention. **Full end-to-end intent + signed-off decisions: [docs/plans/pr-c-news-v0-backtest.md](docs/plans/pr-c-news-v0-backtest.md).** Deletes both plan docs in the cleanup commit.
